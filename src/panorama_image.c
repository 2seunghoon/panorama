...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    qsort(m, an, sizeof(match), &match_compare); // matching들 distance별 오름차순으로 정렬
    for (i = 0; i < an; ++i) {
        if (seen[m[i].bi]) {
            // 중복되는 점이라면
            for (j = i; j < an - 1; ++j) {
                // 해당 matching 버릴것이므로, 한칸씩 당겨줌
                m[j] = m[j + 1];
            }
            i = count - 1; 
            an--;
            ...
            // 중복되지 않은 것이라면
            seen[m[i].bi] = 1; // 이제 봤다고 표시해줌
            count++; // 전체 matching point 개수
        }
    }
    *mn = count;
    free(seen);
...
int model_inliers(matrix H, match *m, int n, float thresh)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (point_distance(p, q) < thresh) {
            // matching point 간의 거리가 threshold보다 작으면 inliner로 취급
            count++;
        }
        else {
...
matrix RANSAC(match *m, int n, float thresh, int k, int cutoff)
{
    ...
    for (int i = 0; i < k; i++) {
        ...
        int inlier = model_inliers(H, m, n, thresh); // inlier들 list 앞쪽으로 정렬됨
        if (inlier > best) {
            best = inlier;
            Hb = compute_homography(m,inlier); // 정렬된 inlier개수만큼 추가로 또 계산하겠다.
        }
        if (best > cutoff) {
            return Hb;
        }
